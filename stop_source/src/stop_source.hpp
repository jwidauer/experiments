...
class stop_token {
 ...
 private:
  ...
  struct binary_semaphore {
    ...
    void release() { _M_counter.fetch_add(1, memory_order_release); }

    void acquire()
    {
      int __old = 1;
      while (
          !_M_counter.compare_exchange_weak(__old, 0, memory_order_acquire, memory_order_relaxed)) {
        __old = 1;
        _S_yield();
      }
    }
  ...
  struct _Stop_cb {
    ...
    [[__gnu__::__nonnull__]] explicit _Stop_cb(__cb_type* __cb) : _M_callback(__cb) {}
  ...
  struct _Stop_state_t {
    ...
    bool _M_stop_possible() noexcept
    {
      ...
      return _M_value.load(memory_order_acquire) & ~_S_locked_bit;
    }

    bool _M_stop_requested() noexcept
    {
      return _M_value.load(memory_order_acquire) & _S_stop_requested_bit;
    }

    void _M_add_owner() noexcept { _M_owners.fetch_add(1, memory_order_relaxed); }

    void _M_release_ownership() noexcept
    {
      if (_M_owners.fetch_sub(1, memory_order_acq_rel) == 1) delete this;
    }

    void _M_add_ssrc() noexcept { _M_value.fetch_add(_S_ssrc_counter_inc, memory_order_relaxed); }

    void _M_sub_ssrc() noexcept { _M_value.fetch_sub(_S_ssrc_counter_inc, memory_order_release); }
    ...
    void _M_lock() noexcept
    {
      ...
      auto __old = _M_value.load(memory_order_relaxed);
      while (!_M_try_lock(__old, memory_order_relaxed)) {
      }
    }

    // Precondition: calling thread holds the lock.
    void _M_unlock() noexcept { _M_value.fetch_sub(_S_locked_bit, memory_order_release); }

    bool _M_request_stop() noexcept
    {
      // obtain lock and set stop_requested bit
      auto __old = _M_value.load(memory_order_acquire);
      do {
      ...
      while (_M_head) {
        ...
        if (!__destroyed) {
          ...
          if (!__gnu_cxx::__is_single_threaded()) __cb->_M_done.release();
        }

        // Avoid relocking if we already know there are no more callbacks.
        if (__last_cb) return true;
    ...
    [[__gnu__::__nonnull__]] bool _M_register_callback(_Stop_cb* __cb) noexcept
    {
      auto __old = _M_value.load(memory_order_acquire);
      do {
        if (__old & _S_stop_requested_bit)  // stop request already made
    ...
    [[__gnu__::__nonnull__]] void _M_remove_callback(_Stop_cb* __cb)
    {
      _M_lock();

      if (__cb == _M_head) {
        _M_head = _M_head->_M_next;
        if (_M_head) _M_head->_M_prev = nullptr;
        _M_unlock();
        return;
      } else if (__cb->_M_prev) {
        __cb->_M_prev->_M_next = __cb->_M_next;
        if (__cb->_M_next) __cb->_M_next->_M_prev = __cb->_M_prev;
        _M_unlock();
        return;
      }
      ...
      if (__cb->_M_destroyed) *__cb->_M_destroyed = true;
    }
    ...
    bool _M_try_lock(value_type& __curval, memory_order __failure = memory_order_acquire) noexcept
    {
      return _M_do_try_lock(__curval, 0, memory_order_acquire, __failure);
    }
    ...
    {
      return _M_do_try_lock(__curval, _S_stop_requested_bit, memory_order_acq_rel,
                            memory_order_acquire);
    }

    bool _M_do_try_lock(value_type& __curval, value_type __newbits, memory_order __success,
  ...
  struct _Stop_state_ref {
    ...
    [[__gnu__::__access__(__none__, 2)]] explicit _Stop_state_ref(const stop_source&)
    ...
    {
      if (_M_ptr) _M_ptr->_M_add_owner();
    }

    _Stop_state_ref(_Stop_state_ref&& __other) noexcept : _M_ptr(__other._M_ptr)
    {
    ...
    _Stop_state_ref& operator=(const _Stop_state_ref& __other) noexcept
    {
      if (auto __ptr = __other._M_ptr; __ptr != _M_ptr) {
        if (__ptr) __ptr->_M_add_owner();
        if (_M_ptr) _M_ptr->_M_release_ownership();
        _M_ptr = __ptr;
      }
      return *this;
    }
...
class stop_source {
 public:
  ...
  {
    if (_M_state) _M_state->_M_add_ssrc();
  }
  ...
  ~stop_source()
  {
    if (_M_state) _M_state->_M_sub_ssrc();
  }

  [[nodiscard]] bool stop_possible() const noexcept { return static_cast<bool>(_M_state); }
  ...
  {
    if (stop_possible()) return _M_state->_M_request_stop();
    return false;
  }

  [[nodiscard]] stop_token get_token() const noexcept { return stop_token{_M_state}; }
...
class [[nodiscard]] stop_callback {
 ...
 public:
      ...
      : _M_cb(std::forward<_Cb>(__cb))
  {
    if (auto __state = __token._M_state) {
      if (__state->_M_register_callback(&_M_cb)) _M_state.swap(__state);
    }
  }

  template <typename _Cb, enable_if_t<is_constructible_v<_Callback, _Cb>, int> = 0>
  explicit stop_callback(stop_token&& __token,
                         _Cb&& __cb) noexcept(is_nothrow_constructible_v<_Callback, _Cb>)
      : _M_cb(std::forward<_Cb>(__cb))
  {
    if (auto& __state = __token._M_state) {
 ...
 private:
  struct _Cb_impl : stop_token::_Stop_cb {
    ...
    explicit _Cb_impl(_Cb&& __cb) : _Stop_cb(&_S_execute), _M_cb(std::forward<_Cb>(__cb))
    {
    }

    _Callback _M_cb;

    [[__gnu__::__nonnull__]] static void _S_execute(_Stop_cb* __that) noexcept
    {
